...
-- PrtSc → select area → save + copy to clipboard
hl.bind("Print", hl.dsp.exec_cmd(
    "mkdir -p ~/Pictures/Screenshots && file=~/Pictures/Screenshots/area-$(date +%Y-%m-%d_%H-%M-%S).png && grim -g \"$(slurp)\" \"$file\" && wl-copy < \"$file\""
))

-- Super + PrtSc → full screen → save + copy to clipboard
hl.bind("SUPER + Print", hl.dsp.exec_cmd(
    "mkdir -p ~/Pictures/Screenshots && file=~/Pictures/Screenshots/screen-$(date +%Y-%m-%d_%H-%M-%S).png && grim \"$file\" && wl-copy < \"$file\""
))
...
hl.bind("XF86AudioMute",        hl.dsp.exec_cmd("wpctl set-mute @DEFAULT_AUDIO_SINK@ toggle; swayosd-client --output-volume mute-toggle"), { locked = true, repeating = true })
...
-- Nautilus — transparent glass
-- Thunar — transparent glass
-- Thunar — glossy glass
hl.window_rule({
    name = "thunar-glossy",
    match = { class = "^thunar$" },
